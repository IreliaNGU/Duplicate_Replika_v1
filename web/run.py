...
class Listener_Thread(threading.Thread):
    def __init__(self, driver, clientsock, email):
        threading.Thread.__init__(self)
        print('listener thread ' + threading.current_thread().name + ' establish.')
        self.flag = 1
        self.driver = driver
    ...
    def getlog(self):
        return "chat_" + str(email) + ".log"

    def run(self):
        # AI_class_name = get_class_AI(driver)
        ...
        while self.flag:
            # 找到最后一条AI发的消息
            # self.driver.save_screenshot('error_fetch.png')
            try:
